function graficar_intervalos(Y, Y_, y_upper, y_lower)
    ...
    fill(x_fill, y_fill, 'black', 'FaceAlpha', 0.3, 'EdgeColor', 'red', 'FaceAlpha', 0.3, 'DisplayName', 'Intervalo de confianza');

    % Configurar el título, los ejes y la leyenda
    xlabel('Tiempo [k]', 'FontSize', 15);
    ylabel('Salida y[k]', 'FontSize', 15);
    title('Predicciones e intervalo de confianza', 'FontSize', 18);
    legend('show');
    ax = gca;
    ax.FontSize = 15;
    set(gcf,'color','w');
