...
for i = [1, 6, 11, min_rmse_idx]
    ...
    ylabel('Índice de sensibilidad', 'FontSize', 15)
    xlabel('Regresor', 'FontSize', 15)
    set(gcf, 'color', 'w');
    % Agregar leyenda solo en la última figura
    if i == min_rmse_idx
        legend('Pertenencia')
    end
    title(['RMSE = ' num2str(rmse_values(i)), ', Regresores: ' num2str(num_iter - i + 1)], 'FontSize', 18);
    grid on
    ax = gca;
    ax.FontSize = 15;
    set(gcf,'color','w');
...
hold off
xlabel('Regresores eliminados', 'FontSize', 15)
...
ax.FontSize = 15;
set(gcf,'color','w');
title('RMSE en función del tiempo (número de iteraciones)', 'FontSize', 18)
