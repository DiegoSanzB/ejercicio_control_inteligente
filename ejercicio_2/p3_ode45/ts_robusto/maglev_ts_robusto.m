...
addpath('ejercicio_2/p3_ode45/ts_robusto/')
addpath('ejercicio_2/toolbox_difuso/')

load('maglev_pd_ode_45_data_ruido.mat')
...
clusters = 4; % numero de clusters elegido anteriormente
...
%% Save Model %%
save('ejercicio_2/p3_ode45/ts_robusto/maglev_ts_model_robusto.mat', 'model');
save('ejercicio_2/p3_ode45/ts_robusto/regresores_eliminados_modelo_ts_robusto.mat', 'eliminated_regressors');

%% Intervalos Difusos %%
addpath('ejercicio_1/fuzzy_ts/')

s = fuzzy_numbers(model, x_optim_val, Y.val, 50, 1, 0.1);
n = length(s);    
s_l = s(1:n/2);
s_u = s(n/2+1:end);
...
%% Guardamos spreads %%
save('ejercicio_2/p3_ode45/ts/spreads_ts_model_robusto.mat', 's');
